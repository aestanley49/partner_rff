...
PartnerDataaaaa <- FormatData(RawData)
write.csv(PartnerDataaaaa, "/usr/local/bin/store/partner_rff/OutputAndInputsForThesis/PartnerData.csv")

adjmatrix <- read_csv("/usr/local/bin/store/partner_rff/output/adjacencymatrix.csv")
two <- read_csv("/usr/local/bin/store/partner_rff/OutputAndInputsForThesis/two.csv")
combo <- read.csv("/usr/local/bin/store/partner_rff/OutputAndInputsForThesis/combodf.csv", stringsAsFactors = FALSE)
```

### Figure 1: Correlation matrix
Informative caption Correlation matrix: This matrix plots the bivariate data, the histogram of each variable and calculates the correlations of bivariate plots with their 95% confidence intervals displayed.
...
Informative caption The number of species a partner is working on: All organizations that have are working on more than one species. This is a subsection of the complete dataset as the vast majority of partners only work on one species. 
...
ggplot(data = g_one, mapping = aes(x=variable, y=value)) + geom_col() + theme(axis.text.x = element_text(angle = 90)) + scale_y_continuous(name ="# of Species", limits=c(0,45)) + scale_x_discrete(name ="Name of Partner")
```

### Number of partners per partner
  #### Same question as above about subsections/scale of zoom 
Informative caption The frequency of the number of partnerships per partner: With the exception of several federal agencies and the 50 odd partners working together on one species, the number of partnerships ranges between 0 and 30. Essentially, most organizations partner with more than one other organization and there are a few outliers with vastly more partnerships. 
```{r no partnerships for each partner, echo=FALSE, eval=FALSE}
...
#try log tranforming 
ggplot(data = explore, mapping = aes(x=log(rssum))) + geom_bar() 
```

### Figure [not yet referenced in text] Number of partners/partner vs number of species/partner
  #### different options here 
Informative caption Number of partnerships per partner v number of species per partner: There is a linear correlation between the number of partnerships per partner and the number of species per partner. The more species a partner is working on, the more likely they are to have more partners. [correct conclusion given that this is heavily informed by outliers?] 
```{r echo=FALSE, eval=FALSE}
...
Informative caption Network Analysis: The number of species that partners are working on together is shown by the a link. The thickness of the link on the axis or edge of the circle indicates how many species the two partners are working on and the color corresponds to a specific partner. [note – need to change names to acronyms] 
...
#two <- noextra %>% filter(value > 2) #remove all partners less than 2   # this is where the csv two came from I wrote it to csv 
#change names to ackronymns 
#two <- two %>% select_all()

#abc <- two %>% select(noquote(order(colnames(two))))
...
#chordDiagram(two, order = c(  "California Department of Fish and Game"    ,"USFWS"               , "The Nature Conservancy"  ,                      "BLM"   ,                                    
 #            "California Department of Fish and Wildlife" , "NPS"                           ,
#"USFS"                         ,             
#        "Nevada Department of Wildlife" ))

chordDiagram(two, scale = FALSE,link.sort = TRUE, link.decreasing = TRUE)
#circos.info()

chordDiagram(two, order = c( "USFWS",    "CDFG"  ,            "NPS",      "BLM" ,                           "The Nature Conservancy"  ,             "USFS"  , "NDW" ))
...
summary table (or section of?)



Residual vs fitted and qqplot 
...
(pdata) ##getting werid error here, look into using a different table format? 
pdata %>% kable("html")

PartnerData[,c(1:3)]  %>% kable("html")
```
