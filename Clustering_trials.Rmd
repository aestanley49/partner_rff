...
******* Do do - color code by org type added to NMDS %^&*(*&^%$%^&*()(*&^%$#$%^&*()))
...
########### text that works in base R
plot(Testnmds) # actions are crosses and partners are circles
ordiplot(Testnmds,type="n")
orditorp(Testnmds,display="species",col="red",air=0.01)
orditorp(Testnmds,display="sites",cex=0.3,air=0.01)
## The above two rows don't work in markdown but will in regular R... ?? 
ordicluster(Testnmds,hclust(as.dist(1-jaccsim), method = "single")) 

ordicluster(Testnmds,hclust(vegdist(TestPartners,"bray"))) ## produces cluster graph with NMDS (same output as from tutorial) 

hclust(vegdist(TestPartners,"bray"))
plot(hclust(vegdist(TestPartners,"bray"))) ### produces dendrogram on partners (not actions)
...
condense <- docspef[,-c(2)] #select relevant columns
