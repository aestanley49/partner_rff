...
ggplot(smelt) + aes(x=variable, y=value, fill=rowname) +
...
ggplot(sactions_melt) + aes(x=variable, y=value, fill=rowname) +
  geom_bar(stat="identity") + 
  scale_x_discrete(name ="species") + 
...
ggplot(act_land) + aes(x=variable, y=value, fill=actionresults) +
  geom_bar(stat="identity") + 
...
ggplot(threats_actions) + aes(x=variable, y=value, fill=rowname) +
  geom_bar(stat="identity") + 
...
diff_tb <- data.frame('preempt' = c(13,18,19), 'respon' = c(1,12,13), 'unc' = c(7,13,0))
...
test relationship between number of iniatives and number of partners 
```{r}
