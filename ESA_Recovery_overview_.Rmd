...
lme <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + public_mang_shannons_diversity ,data = reg_prdctr_init)
...
mdls <- list(
  lm10, lm14, lmc, lmd, lme
)


stargazer(lm10, lm14, lmc, lmd, lme, 
          type = "html", 
          title = "Initiatives_models_Supplementary", 
          align = TRUE, 
          out = "output/Initiatives_models_1_Aug_2021.html", 
          flip = TRUE, # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", aic_calc(lm10), aic_calc(lm14), aic_calc(lmc), aic_calc(lmd), aic_calc(lme)  )) , 
          ...
          covariate.labels=c("Log of range area", "Plant (present = 1)", "Total number of five factor threats", "NatureServe popn stability",  "Proportion of private land",  "wd proposed","month 12 findings", "Public mang. shannons diversity" ,"Constant"),
...
taxa <- outcomes
taxa <- taxa %>% dplyr::select(scientific_name, common_name, preempt_private:species_group, preempt:landresults)
...
taxa_small <- taxa %>% dplyr::select(scientific_name, actionresults, vert_vs_invert )
...
taxa_land <- taxa %>% dplyr::select(scientific_name, landresults, vert_vs_invert )
...
land_preempt <- land_preempt %>% dplyr::select(scientific_name, actionresults, landresults)
...
range <- outcomes
range <- range %>% dplyr::select(scientific_name, range_area, actionresults)
...
threat_type <- threat_type %>% dplyr::select(scientific_name, habitat_modification:total_threats, actionresults)
...
threats_plot <- ggplot(threat_actions1) + aes(x=two, y=threat_actions, fill = (three)) +
  geom_bar(stat="identity", position = "stack", width = 1, preserve = "single") + 
  facet_wrap(~one, nrow = 1) +
  ggtitle("Actions for types of threats") + 
  scale_y_continuous(name = "Count")+
  labs(fill = "Yes or No")+ 
...
lm18 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + public_mang_shannons_diversity, data = reg_partners)
...
lm20 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + wd_proposed + month_12 + public_mang_shannons_diversity + total_threats , data = reg_partners) #kitchen sink


lm21 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + public_mang_shannons_diversity, data = reg_partners)

lm22 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + Shannon_private, data = reg_partners)

lm23 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + Shannon_private_area, data = reg_partners)
...
mdls <- list(
  lm16, lm10, lm18, lm19, lm20, lm21, lm22, lm23
)
...
stargazer(lm16, lm10, lm18, lm19, lm20, lm21, lm22, lm23, 
          type = "html", 
          title = "Partners_models_Supplementary", 
          align = TRUE, 
          out = "output/Partners_models_1_Aug_2021.html", 
          flip = TRUE, # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", aic_calc(lm16), aic_calc(lm10), aic_calc(lm18), aic_calc(lm19), aic_calc(lm20), aic_calc(lm21) , aic_calc(lm22) , aic_calc(lm23)  )) , 
          se = lapply(mdls, se_robust),  ## setting se to be calculated by Robust Standard Errors from function
          omit.stat = c("f", "ser"), 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)", "NatureServe popn stability", "Total number of five factor threats",  "Proportion of private land", "Public mang. shannons diversity" , "wd proposed","month 12 findings", "Constant"),
...
stargazer(lm10, type = "html", title = "Preemptive Conservation Models",  out = "output/format_triala.html", 
          ...
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)", "Total number of five factor threats",  "Proportion of private land", "Constant"),
          dep.var.labels=c("Log of No. Initiatives"), 
...
part_type <- part_type %>% dplyr::select(scientific_name, common_name, RowSpecInfo, Agreement_Name, Partner_Source, Partners, Type_of_Org )
...
stargazer(lma1, lmb1, lma2, lmb2, type = "html", title = "Preemptive Conservation Models Confidence Check",  out = "output/WeightedConfidenceCheck.html", 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)",  "Natureserve popn. stability", "Total 5 factor threats", "Proportion of private land", "Constant"),
          column.labels = c("PartnerNormal", "Partner.5", "Partner1.5", "InitiativesNormal", "Initiatives.5", "Initiatives1.5"),
          model.numbers = FALSE,
