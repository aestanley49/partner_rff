...
```{r, include = FALSE, echo = FALSE}
stargazer(lm1, lm2, lm4, lm6, lm8, lm9, type = "html", title = "Initiatives_models", align = TRUE, out = "output/Initiatives_models_22_Jun_2021.html", # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", round(AIC(lm1),1), round(AIC(lm2),1), round(AIC(lm4),1), round(AIC(lm6),1), round(AIC(lm8),1), round(AIC(lm9),1)  ))) 
...
reg_prdctr_partners <- reg_prdctr_partners %>% select(-percentpublic, -no_iniv)
...
reg_prdctr_partners <- reg_prdctr_partners %>% select(scientific_name:common_name , -normal_count:FR_only_count, log_normal:log_FR, log_rangearea, range_area:wd_proposed)
...
VIFS
```{r, include = FALSE, echo = FALSE}
vif(lm(log(normal_count) ~ range_area + plant + public_mang_shannons_diversity + total_threats,data = reg_partners))

vif(lm(log(no_offices) ~ range_area + plant + public_mang_shannons_diversity + total_threats,data = reg_partners))

vif(lm(log(NoHCPS_count) ~ range_area + plant + public_mang_shannons_diversity + total_threats,data = reg_partners))

vif(lm(log(FR_only_count) ~ range_area + plant + public_mang_shannons_diversity + total_threats,data = reg_partners))
```
...
```{r, include = FALSE, echo = FALSE}
stargazer(lm1, lm2, lm4, lm6, lm8, lm9, type = "html", title = "Partners_models", align = TRUE, out = "output/Partners_models_22_Jun_2021.html", # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", round(AIC(lm1),1), round(AIC(lm2),1), round(AIC(lm4),1), round(AIC(lm6),1), round(AIC(lm8),1), round(AIC(lm9),1)  ))) 
