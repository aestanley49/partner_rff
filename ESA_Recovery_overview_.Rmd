...
library(sjPlot) # like stargazer but tables look more like what we want
library(sandwich) ## for Robust Standard Errors and testing heteroskedasticty 
library(lmtest) ## not sure what this does but was on same blog as sandwich package
```

##load in different datasets
```{r, echo = FALSE, include = FALSE}

newthreatsdata <- read_excel(paste0(DataSource, "/AS13_RSE2_May_6_22_codingdata.xlsx"), sheet = "threats", trim_ws = T, col_names = T, na = c("", " ", "NA") #issue was that "" is coming up instead of NA
               )

initiatves_partners <- read_excel(paste0(DataSource, "/AS12_Jun_15_21_codingdata.xlsx"), sheet = "initiatves_partners", trim_ws = T, col_names = T, na = c("", " ", "NA") #issue was that "" is coming up instead of NA
               )

initiatves_partners2 <- read_excel(paste0(DataSource,"/AS12_Aug_1_21_codingdata.xlsx"), sheet = "initiatves_partners", trim_ws = T, col_names = T, na = c("", " ", "NA") #issue was that "" is coming up instead of NA
               ) ## This has the updated Partner types but it breaks my code if I change all to this format

proactive <- read_excel(paste0(DataSource, "/AS12_Jun_15_21_codingdata.xlsx"), sheet = "threats", trim_ws = T, col_names = T, na = c("", " ", "NA") #issue was that "" is coming up instead of NA
               )

confidence <- read_excel(paste0(DataSource, "/AS12_Jun_15_21_codingdata.xlsx"), sheet = "metadata", trim_ws = T, col_names = T, na = c("", " ", "NA") #issue was that "" is coming up instead of NA
               )
...
proactive <- left_join(names, newthreatsdata)

FR_pg_length_and_doc_type <- proactive %>% dplyr::select(scientific_name, common_name, FR_pg_length, type_FR_doc)
...
t_pro_df <- t_pro_df %>% dplyr::select(-common_name.y)
t_pro_df$common_name <- t_pro_df$common_name.x
t_pro_df <- t_pro_df %>% dplyr::select(-common_name.x)
...
init_type_sp_count <- no_init_per_sp %>% 
  group_by(RowSpecInfo) %>% dplyr::summarise(n())

no_init_partner <- sub_initiatves_partners %>% group_by(common_name, RowSpecInfo, Partners) %>% distinct(Agreement_Name) %>%    #count how many names are distinct 
dplyr::summarise(n())

no_init_partner_2 <- no_init_partner %>% 
  group_by(RowSpecInfo) %>% dplyr::summarise(n())
...
reg_prdctr <- L_df %>% dplyr::select(scientific_name, common_name, no_iniv, range_area, plant, public_mang_shannons_diversity, total_threats, private_land_proportion, weighted_rural)
...
FR_info  <- FR_info %>% dplyr::select(-FR_pg_length, -type_FR_doc)             
...
reg_prdctr_init <- reg_prdctr_init %>% dplyr::select(scientific_name:no_iniv, log_initatives, log_rangearea, range_area:Big_Pg_count)

## removing %public until have all data and Big_pg_count 
reg_prdctr_init <- reg_prdctr_init %>% dplyr::select(-Big_Pg_count)


## Need an extra variable to try in the partners model so pulling it here
ns_threats <-  dplyr::select(tdata, common_name, scientific_name, nserve_pop_stability)

reg_prdctr_init <- reg_prdctr_init %>% left_join(ns_threats, by = "scientific_name")

reg_prdctr_init <- reg_prdctr_init %>% dplyr::select(-common_name.y)

## need to get same number of observations for all models so remove nas from nserve

##reg_prdctr_init <- reg_prdctr_init[-which(is.na(reg_prdctr_init$nserve_pop_stability)),]

reg_prdctr_init <- reg_prdctr_init %>% mutate(Shannon_private = ((1 - private_land_proportion)*public_mang_shannons_diversity)) %>% mutate(Shannon_private_area = ((1 - private_land_proportion)*log_rangearea *public_mang_shannons_diversity))
...
#set up
PredictorsOnlyPixel <- reg_prdctr_init[,c(4)]
PredictAndResponsePixel <- reg_prdctr_init[,-c(1:3,6,11)]
PredictAndResponseGrid <- reg_prdctr_init[,-c(1:3,6,11)]
...
vif(lm(log(no_iniv) ~ range_area + plant  + total_threats + private_land_proportion, data = reg_prdctr_init))
...
lmc <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + nserve_pop_stability + private_land_proportion , data = reg_prdctr_init)

lmd <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + wd_proposed + month_12 + nserve_pop_stability,data = reg_prdctr_init) #kitchen sink


#lme <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + public_mang_shannons_diversity ,data = reg_prdctr_init) #no longer want shannon's H

lmf <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + Shannon_private,data = reg_prdctr_init)

lmg <- lm(log(no_iniv) ~  plant  + total_threats  + Shannon_private_area,data = reg_prdctr_init)

#lmh <- lm(log(no_iniv) ~ plant  + total_threats + private_land_proportion + Log_Shannon_area,data = reg_prdctr_init)
...
lm14 <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + wd_proposed + month_12,data = reg_prdctr_init)
lma <- lm(log(no_iniv) ~ log(range_area) + plant + nserve_pop_stability,data = reg_prdctr_init)
lmb <- lm(log(no_iniv) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion, data = reg_prdctr_init)
lmc <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + nserve_pop_stability + private_land_proportion , data = reg_prdctr_init)
...
coeftest(lm10, vcov = vcovHC(lm10, type="HC1"))
```

Trying stargazer 

```{r stargazer intiative, include = FALSE, echo = FALSE}

lm10a <- lm((no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion,data = reg_prdctr_init)
lm10 <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion,data = reg_prdctr_init)

lmc <- lm(log(no_iniv) ~ log(range_area) + plant + total_threats  + nserve_pop_stability + private_land_proportion , data = reg_prdctr_init)

lm20 <- lm(log(no_iniv) ~ log(range_area) + plant   + nserve_pop_stability + private_land_proportion , data = reg_prdctr_init)

lm14 <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + wd_proposed + month_12,data = reg_prdctr_init)

lmd <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion + wd_proposed + month_12 + nserve_pop_stability,data = reg_prdctr_init) #kitchen sink

lm22 <- lm(log(no_iniv) ~ log(range_area) + plant  + private_land_proportion + wd_proposed + month_12 + nserve_pop_stability,data = reg_prdctr_init) #kitchen sink
...
mdls <- list(
   lm10,  lm20, lmc, lm14, lm22, lmd
)


stargazer(lm10, lm20, lmc, lm14, lm22, lmd, 
          type = "html", 
          
          align = TRUE, 
          out = "output/Initiatives_models_16_Jun_2022.html", 
          flip = TRUE, # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", aic_calc(lm10), aic_calc(lm20), aic_calc(lmc), aic_calc(lm14), aic_calc(lm22), aic_calc(lmd) )) , 
          se = lapply(mdls, se_robust),  ## setting se to be calculated by Robust Standard Errors from function
          omit.stat = c("f", "ser"), 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)", "Total threats", "NatureServe popn stability",  "Proportion of private land",  "Withdrawal of a proposed rule","12 Month finding", "Constant"),
          dep.var.labels=c("Log of No. Initiatives") )



#stargazer(lm10, lm14, lmc, lmd, lme,lmf,lmg, type = "html", title = "Initiatives_models_Supplementary", align = TRUE, out = #"output/Initiatives_models_25_Jul_2021.html", # AIC isn't calculated for lm models so adding manually
#          add.lines=list(c("AIC", round(AIC(lm10),1), round(AIC(lm14),1), round(AIC(lmc),1), round(AIC(lmd),1), round(AIC(lme),1), round(AIC(lmf),1), round(AIC(lmg),1)
#                           ))) 


#stargazer(lm1, lm2, lm10, lm11, lm12, lm13, lm14, lmb, lmc, type = "html", title = "Initiatives_models", align = TRUE, out = "output/Initiatives_models_22_Jun_2021.html", # AIC isn't calculated for lm models so adding manually
 #         add.lines=list(c("AIC", round(AIC(lm1),1), round(AIC(lm2),1), round(AIC(lm10),1), round(AIC(lm11),1), round(AIC(lm12),1), round(AIC(lm13),1), round(AIC(lm14),1) , round(AIC(lmb),1) , round(AIC(lmc),1) ))) 
...
- 
```{r predicting values 1, include = FALSE, echo = FALSE}
...
lm10 <- lm(log(no_iniv) ~ log_rangearea + plant  + total_threats + private_land_proportion,data = reg_prdctr_init)

summary(lm10)

## Get mean from summary stats of each predictor
summary(reg_prdctr_init$log_rangearea)
summary(reg_prdctr_init$plant)
summary(reg_prdctr_init$total_threats)
summary(reg_prdctr_init$private_land_proportion)

summary(reg_prdctr_init$range_area)
meanlog <- log(3.050e+10) #mean for range area logged #24.14099
meanlogx2 <- log((3.050e+10) * 2)
meanlogx1_2 <- log((3.050e+10)*.5)

#predict(lm10, data.frame(log_rangearea = 23.39, plant = 0.3571, total_threats =  2.333, private_land_proportion = 0.4871))

#summary(lm10)

base <- predict(lm10, data.frame(log_rangearea = meanlog, plant = 0.3571, total_threats =  2.333 , private_land_proportion = 0.4871))
base <- exp(base)
base <- base %>% as.data.frame() %>% add_column("base")

plant_0 <- predict(lm10, data.frame(log_rangearea = meanlog, plant = 0, total_threats =  2.333 , private_land_proportion = 0.4871))
plant_0 <- exp(plant_0)
plant_0 <- plant_0 %>% as.data.frame() %>% add_column("plant_0")

plant_1 <- predict(lm10, data.frame(log_rangearea = meanlog, plant = 1, total_threats =  2.333 , private_land_proportion = 0.4871))
plant_1 <- exp(plant_1)
plant_1 <- plant_1 %>% as.data.frame() %>% add_column("plant_1")

log_rangeareax2 <- predict(lm10, data.frame(log_rangearea = meanlogx2, plant = 0.3571, total_threats =  2.333 , private_land_proportion = 0.4871)) ## big change here 
log_rangeareax2 <- exp(log_rangeareax2)
log_rangeareax2 <- log_rangeareax2 %>% as.data.frame() %>% add_column("log_rangeareax2")

log_rangeareax1_2 <- predict(lm10, data.frame(log_rangearea = meanlogx1_2, plant = 0.3571, total_threats =  2.333 , private_land_proportion = 0.4871))
log_rangeareax1_2 <- exp(log_rangeareax1_2)
log_rangeareax1_2 <- log_rangeareax1_2 %>% as.data.frame() %>% add_column("log_rangeareax1_2")

private_land_proportionx2 <- predict(lm10, data.frame(log_rangearea = meanlog, plant = 0.3571, total_threats =  2.333 , private_land_proportion = (0.4871 * 2)))
private_land_proportionx2 <- exp(private_land_proportionx2)
private_land_proportionx2 <- private_land_proportionx2 %>% as.data.frame() %>% add_column("private_land_proportionx2")

private_land_proportionx1_2 <- predict(lm10, data.frame(log_rangearea = meanlog, plant = 0.3571, total_threats =  2.333 , private_land_proportion = (0.4871 * .5)))
private_land_proportionx1_2 <- exp(private_land_proportionx1_2)
...
predictions <- base %>% full_join(plant_0) %>% full_join(plant_1) %>% full_join(log_rangeareax2) %>% full_join(log_rangeareax1_2) %>% full_join(private_land_proportionx2) %>% full_join(private_land_proportionx1_2)

## this is clunky but it works .. 

diagn <- diag(as.matrix(predictions[, -1]))
predictions$call <- diagn

names(predictions) <- c(
 "value"          ,                     "\"base\""    ,                    
 "\"plant_0\""          ,           "\"plant_1\""     ,               
           "\"log_rangeareax2\""      ,      
 "\"log_rangeareax1_2\""    ,       "\"private_land_proportionx2\""  ,
  "\"private_land_proportionx1_2\"" , "call" 
)


predictions <- predictions[,c(1,9)]

#predict_values <- predictions %>% select(value, call)
## Can't knit this line ^^ but this gets final dataset 
```
...
### Testing to see wether there is action info for all 42 species
code_test <- outcomes[,c(1:10)]
code_test <- code_test %>% rowwise() %>% mutate("count" = sum(preempt_private, preempt_public, preempt_unclear, respon_private, respon_public, respon_unclear, private_unclear, public_unclear)) 
## 1 species doesn't have any action info.. (BASALT DAISY)

### remove row with all 0s (this will change things aka (BASALT DAISY) ) 
outcomes <- outcomes[-c(4),]


all <- all %>% dplyr::select(preempt:react, public:private)
...
test_land <- all2 %>% filter(one == "private" | one =="public") %>% dplyr::select(yes, no)
...
test_actions
## Total number y/n preempted and reactive 
...
test_land_a
## Total number y/n private and public 
...
taxa2 <- taxa2 %>% dplyr::select(scientific_name, common_name, vert_vs_invert:species_group, habitat_modification:demographic_stochasticity,  preempt:react, public:private)
...
taxa_actions <- taxa2
taxa_actions <- taxa_actions %>% dplyr::select(plant_preempt_yes, plant_preempt_no, plant_react_yes, plant_react_no, vert_preempt_yes, vert_preempt_no, vert_react_yes, vert_react_no, invert_preempt_yes, invert_preempt_no , invert_react_yes, invert_react_no)
...
test_taxa_actions <- taxa_actions2 %>% dplyr::select(yes, no) # Have a zero in dataset
...
taxa_actions2 %>% filter(one == "plant") %>% kable()

taxa_plant <- taxa_actions2 %>% filter(one == "plant") %>% dplyr::select(yes, no)
fish_taxa_plant <- pairwise_fisher_test(taxa_plant, p.adjust.method = "bonferroni")
fish_taxa_plant <- fisher_test(taxa_plant)
fish_taxa_plant

taxa_actions2 %>% filter(one == "vert") %>% kable()

taxa_vert <- taxa_actions2 %>% filter(one == "vert") %>% dplyr::select(yes, no)
fish_taxa_vert <- pairwise_fisher_test(taxa_vert, p.adjust.method = "bonferroni")
fish_taxa_vert <- fisher_test(taxa_vert)
fish_taxa_vert

taxa_actions2 %>% filter(one == "invert") %>% kable()

taxa_invert <- taxa_actions2 %>% filter(one == "invert") %>% dplyr::select(yes, no)
fish_taxa_invert <- pairwise_fisher_test(taxa_invert, p.adjust.method = "bonferroni")
fish_taxa_invert <- fisher_test(taxa_invert)
fish_taxa_invert



taxa_actions2 %>% filter(two == "preempt") %>% kable()

taxa_preempt <- taxa_actions2 %>% filter(two == "preempt") %>% dplyr::select(yes, no)
fish_taxa_preempt <- pairwise_fisher_test(taxa_preempt, p.adjust.method = "bonferroni")
#fish_taxa_preempt <- fisher_test(taxa_preempt)
fish_taxa_preempt

taxa_actions2 %>% filter(two == "react") %>% kable()

taxa_react <- taxa_actions2 %>% filter(two == "react") %>% dplyr::select(yes, no)
...
taxa_land <- taxa2
taxa_land <- taxa_land %>% dplyr::select(plant_public_yes, plant_public_no, plant_private_yes, plant_private_no, vert_public_yes, vert_public_no, vert_private_yes, vert_private_no, invert_public_yes, invert_public_no , invert_private_yes, invert_private_no)
...
test_taxa_land <- taxa_land2 %>% dplyr::select(yes, no) # Have a zero in dataset
...
ggplot(taxa_land1) + aes(x=two, y=taxa_land, fill = (three)) +
  geom_bar(stat="identity", position = stack(preserve = "single"), width = .9) +  ###??????
  facet_wrap(~one) +
...
taxa_land2 %>% filter(one == "plant") %>% kable()

taxa_plant <- taxa_land2 %>% filter(one == "plant") %>% dplyr::select(yes, no)
fish_taxa_plant <- pairwise_fisher_test(taxa_plant, p.adjust.method = "bonferroni")
fish_taxa_plant <- fisher_test(taxa_plant)
fish_taxa_plant


taxa_land2 %>% filter(one == "vert") %>% kable()

taxa_vert <- taxa_land2 %>% filter(one == "vert") %>% dplyr::select(yes, no)
fish_taxa_vert <- pairwise_fisher_test(taxa_vert, p.adjust.method = "bonferroni")
fish_taxa_vert <- fisher_test(taxa_vert)
fish_taxa_vert


taxa_land2 %>% filter(one == "invert") %>% kable()

taxa_invert <- taxa_land2 %>% filter(one == "invert") %>% dplyr::select(yes, no)
fish_taxa_invert <- pairwise_fisher_test(taxa_invert, p.adjust.method = "bonferroni")
fish_taxa_invert <- fisher_test(taxa_invert)
fish_taxa_invert


taxa_land2 %>% filter(two == "public") %>% kable()

taxa_public <- taxa_land2 %>% filter(two == "public") %>% dplyr::select(yes, no)
...
taxa_land2 %>% filter(two == "private") %>% kable()

taxa_private <- taxa_land2 %>% filter(two == "private") %>% dplyr::select(yes, no)
...
#fish_taxa_private <- fisher_test(taxa_private)
 ...
 taxa_land1a <- taxa_land1 %>%    mutate(one = case_when(str_detect(one, 'plant') ~ 'Plant', 
                           str_detect(one, '^vert') ~ 'Vertebrate',
                           str_detect(one, 'inver') ~ 'Invertebrate')) %>% 
                          mutate(two = case_when(str_detect(two, 'public') ~ 'Public', 
                           str_detect(two, 'private') ~ 'Private'))

ggplot(taxa_land1a) + aes(x=two, y=taxa_land, fill = (three)) +
  ...
  labs(fill = "Yes or No")+ 
  theme(panel.margin = grid:: unit(-1.25, "lines"))  +
  scale_x_discrete(name = "Type of Action") + 
...
landactions <- landactions %>% dplyr::select(scientific_name, common_name,  preempt:react, public:private)
...
landactions <- landactions %>%  
 ...
 ## # of species with at least one restorative action and at least one prohibitive action mentioned 
actions_mentioned <- colSums(landactions[, c(3,4)], na.rm = T)


landactionsa <- landactions
landactionsa <- landactionsa %>% dplyr::select(-scientific_name:-private)
...
test_landactions <- landactions2 %>% dplyr::select(yes, no) # Have a zero in dataset
...
landactions2 %>% filter(one == "preempt") %>% kable()

landactions_preempt <- landactions2 %>% filter(one == "preempt") %>% dplyr::select(yes, no)
...
fish_landactions_preempt


landactions2 %>% filter(one == "react") %>% kable()

landactions_react <- landactions2 %>% filter(one == "react") %>% dplyr::select(yes, no)
...
landactions1 %>% filter(two == "public") %>% kable()

landactions_preempt <- landactions2 %>% filter(two == "public") %>% dplyr::select(yes, no)
fish_landactions_preempt <- pairwise_fisher_test(landactions_preempt, p.adjust.method = "bonferroni")
fish_landactions_preempt <- fisher_test(landactions_preempt)
fish_landactions_preempt


landactions2 %>% filter(two == "private") %>% kable()

landactions_react <- landactions2 %>% filter(two == "private") %>% dplyr::select(yes, no)
fish_landactions_react <- pairwise_fisher_test(landactions_react, p.adjust.method = "bonferroni")
fish_landactions_react <- fisher_test(landactions_react)
fish_landactions_react
...
types <- types %>% dplyr::select(scientific_name, common_name, vert_vs_invert:species_group, habitat_modification:demographic_stochasticity,  preempt:react, public:private)
...
threat_actions <- types %>% dplyr::select(-scientific_name:-private)
...
test_threat_actions <- threat_actions2 %>% dplyr::select(yes, no) # Have a zero in dataset
test <- chisq.test(test_threat_actions)
test



threat_actions1 <- threat_actions1 %>%
  mutate(two = case_when(str_detect(two, 'preempt') ~ 'Restorative ', 
                           str_detect(two, 'react') ~ 'Prohibitive',
                           TRUE ~ 'OTHER')) %>%
  mutate(one = case_when(str_detect(one, 'hab') ~ 'Habitat Modification', 
                           str_detect(one, 'overut') ~ 'Overutilization',
                         str_detect(one, 'poll') ~ 'Pollution',
                         str_detect(one, 'spsp') ~ 'Species–Species Interaction',
                         str_detect(one, 'envst') ~ 'Environmental Stochasticity',
                         str_detect(one, 'demst') ~ 'Demographic Stochasticity',
                           TRUE ~ 'OTHER'))


threats_plot <- ggplot(threat_actions1) + aes(x=two, y=threat_actions, fill = (three)) +
  geom_bar(stat="identity", position = "stack", width = .9) + 
  facet_wrap(~one, nrow = 1) +
...
ggsave(filename = "threats_plot_Aug_8.png",threats_plot , device = 'png')

#fish <- row_wise_fisher_test(test_threat_actions, workspace = 2e8, p.adjust.method = "bonferroni")
#fish

threat_actions2 %>% filter(two == "preempt") %>% kable()

threat_preempt <- threat_actions2 %>% filter(two == "preempt") %>% dplyr::select(yes, no)
...
threat_actions2 %>% filter(two == "react") %>% kable()

threat_react <- threat_actions2 %>% filter(two == "react") %>% dplyr::select(yes, no)
...
threat_actions2 %>% filter(one == "hab") %>% kable()

threat_hab <- threat_actions2 %>% filter(one == "hab") %>% dplyr::select(yes, no)
fish_threat_hab <- pairwise_fisher_test(threat_hab, p.adjust.method = "bonferroni")
fish_threat_hab <- fisher_test(threat_hab)
fish_threat_hab


threat_actions2 %>% filter(one == "overut") %>% kable()

threat_overut <- threat_actions2 %>% filter(one == "overut") %>% dplyr::select(yes, no)
...
threat_actions2 %>% filter(one == "poll") %>% kable()

threat_poll <- threat_actions2 %>% filter(one == "poll") %>% dplyr::select(yes, no)
fish_threat_poll <- pairwise_fisher_test(threat_poll, p.adjust.method = "bonferroni")
fish_threat_poll <- fisher_test(threat_poll)
fish_threat_poll


threat_actions2 %>% filter(one == "spsp") %>% kable()

#### This is the only one sig 
threat_spsp <- threat_actions2 %>% filter(one == "spsp") %>% dplyr::select(yes, no)
fish_threat_spsp <- pairwise_fisher_test(threat_spsp, p.adjust.method = "bonferroni")
fish_threat_spsp <- fisher_test(threat_spsp)
fish_threat_spsp


threat_actions2 %>% filter(one == "envst") %>% kable()

threat_envst <- threat_actions2 %>% filter(one == "envst") %>% dplyr::select(yes, no)
fish_threat_envst <- pairwise_fisher_test(threat_envst, p.adjust.method = "bonferroni")
fish_threat_envst <- fisher_test(threat_envst)
fish_threat_envst


threat_actions2 %>% filter(one == "demst") %>% kable()

threat_demst <- threat_actions2 %>% filter(one == "demst") %>% dplyr::select(yes, no)
...
## change names of threats and action types 


threats_plot <- ggplot(threat_actions1) + aes(x=two, y=threat_actions, fill = (three)) +
  geom_bar(stat="identity", position = "stack", width = 1, preserve = "single", color = "black") + 
  facet_wrap(~one, nrow = 1) +
  ggtitle("Actions for types of threats") + 
  scale_y_continuous(name = "Count")+
  labs(fill = "Yes or No")+ 
...
land_taxa <- sactions
land_taxa <- land_taxa %>% dplyr::select(-rowname)
rname <- rownames(land_taxa)

#add vert and invert to see if can get values up for chi square
land_taxa <- land_taxa %>% mutate(allvert = vert + invert)
land_taxa <- land_taxa %>% dplyr::select(-vert, -invert)
...
normal_partner_count <- partners %>% group_by(scientific_name) %>% 
  distinct(Partners) %>%  dplyr::summarise(n()) 
...
no_district_part_count <- no_district_part %>% group_by(scientific_name) %>% 
  distinct(Partners) %>%  dplyr::summarise(n())  #count modified number of partners here now
...
## Need an extra variable to try in the partners model so pulling it here
ns_threats <- tdata %>% dplyr::select(common_name, scientific_name, nserve_pop_stability)
...
## re arrange order 
reg_prdctr_partners <- reg_prdctr_partners %>% dplyr::select(scientific_name:no_iniv, log_rangearea, range_area:Big_Pg_count, nserve_pop_stability)
## removing %public until have all data and
reg_prdctr_partners <- reg_prdctr_partners %>% dplyr::select( -no_iniv)
...
reg_partners <- reg_partners %>% dplyr::select(scientific_name, common_name.x, NoHCPS_count, log_rangearea:nserve_pop_stability) %>% dplyr::select(-weighted_rural)
...
#reg_prdctr_partners <- reg_prdctr_partners %>% dplyr::select(scientific_name:common_name , normal_count:FR_only_count, log_rangearea, range_area:wd_proposed)
...
reg_partners <- reg_partners %>% mutate(log_HCPS = log(NoHCPS_count))

## add two new variables

reg_partners <- reg_partners %>% mutate(Shannon_private = ((1 - private_land_proportion)*public_mang_shannons_diversity)) %>% mutate(Shannon_private_area = ((1 - private_land_proportion)*log_rangearea *public_mang_shannons_diversity))



```

Writing csv of the datatable for the 42 species for tyler 
```{r}
write.csv(reg_partners, paste(DataSource,"/42species.csv", sep = ""))
```
...
PredictAndResponsePixel <- reg_partners[,-c(1:2, 5, 10, 11, 12, 14)]
PredictAndResponseGrid <- reg_partners[,-c(1:2, 5, 10, 11, 12, 14)]
...
vif(lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion, data = reg_partners))

```


*Note - lm23 is parsimonious with 16 (added log shannon's h)
```{r, include = FALSE, echo = FALSE}
...
#lm18 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion , data = reg_partners)

lm19 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + wd_proposed + month_12, data = reg_partners)

lm20 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + wd_proposed + month_12 + total_threats , data = reg_partners) #kitchen sink


#lm21 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion , data = reg_partners)

lm22 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability, data = reg_partners)

#lm23 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability , data = reg_partners)
...
```{r stargazer partners, include = FALSE, echo = FALSE}
## Old 
...
lmzz <- lm(log(NoHCPS_count) ~  log(range_area)+ plant  + total_threats + nserve_pop_stability + private_land_proportion,data = reg_partners)

lm19 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + wd_proposed + month_12, data = reg_partners)
lm20 <- lm(log(NoHCPS_count) ~ log(range_area) + plant + nserve_pop_stability + private_land_proportion + wd_proposed + month_12 + total_threats , data = reg_partners) #kitchen sink

lm22 <- lm(log(NoHCPS_count) ~ log(range_area) + plant  + private_land_proportion + wd_proposed + month_12 + total_threats , data = reg_partners) #kitchen sink
...
mdls <- list(
  lm16, lm10, lmzz, lm19, lm22, lm20
)

stargazer(
  mdls, type = "text", single.row = TRUE, report = "vcsp", 
  se = lapply(mdls, se_robust), out = "output/Partners_models_SE_trial.html")


stargazer(lm16, lm10, lmzz, lm19, lm22, lm20, 
          type = "html", 
          
          align = TRUE, 
          out = "output/Partners_models_16_Jun_2022.html", 
          flip = TRUE, # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", aic_calc(lm16), aic_calc(lm10), aic_calc(lm19), aic_calc(lmzz), aic_calc(lm20), aic_calc(lm22)   )) , 
          se = lapply(mdls, se_robust),  ## setting se to be calculated by Robust Standard Errors from function
          omit.stat = c("f", "ser"), 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)", "NatureServe popn stability", "Total threats",  "Proportion of private land" , "Withdrawal of a proposed rule","12 Month finding", "Constant"),
          dep.var.labels=c("Log of No. Partners") )
...
Robust Standard Errors in R
```{r Robust Standard Errors, echo=FALSE, warning=FALSE, include=TRUE}

coeftest(lm16, vcov = vcovHC(lm16, type="HC1"))


 coeftest(lm16, vcov. = sandwich::sandwich)[, 2]
 
  coeftest(lm16, vcov = vcovHC(lm16, type="HC1"))[, 2]
```
...
- 
```{r predicting values 2, include = FALSE, echo = FALSE}


## rerunning model so can have log_rangearea instead of log(range_area) --> same values but can't use predict with log(range_area) and get different set up in stargazer if run one model with log_rangearea
lm16 <- lm(log(NoHCPS_count) ~ log_rangearea + plant + nserve_pop_stability + private_land_proportion, data = reg_partners)
...
## Get mean from summary stats of each predictor
summary(reg_partners$range_area)
meanlog <- log(3.050e+10) #mean for range area logged #24.14099
meanlogx2 <- log((3.050e+10) * 2)
meanlogx1_2 <- log((3.050e+10)*.5)
summary(reg_partners$plant)
summary(reg_partners$nserve_pop_stability)
summary(reg_partners$private_land_proportion)


#predict(lm10, data.frame(log_rangearea = 23.39, plant = 0.3571, nserve_pop_stability = 4, private_land_proportion = 0.4871))

#summary(lm10)

base <- predict(lm16, data.frame(log_rangearea = meanlog, plant = 0.3571, nserve_pop_stability = 4 , private_land_proportion = 0.4871))
base <- exp(base)
base <- base %>% as.data.frame() %>% add_column("base")

plant_0 <- predict(lm16, data.frame(log_rangearea = meanlog, plant = 0, nserve_pop_stability = 4 , private_land_proportion = 0.4871))
plant_0 <- exp(plant_0)
plant_0 <- plant_0 %>% as.data.frame() %>% add_column("plant_0")

plant_1 <- predict(lm16, data.frame(log_rangearea = meanlog, plant = 1, nserve_pop_stability = 4 , private_land_proportion = 0.4871))
plant_1 <- exp(plant_1)
plant_1 <- plant_1 %>% as.data.frame() %>% add_column("plant_1")

log_rangeareax2 <- predict(lm16, data.frame(log_rangearea = meanlogx2, plant = 0.3571, nserve_pop_stability = 4 , private_land_proportion = 0.4871)) ## big change here 
log_rangeareax2 <- exp(log_rangeareax2)
log_rangeareax2 <- log_rangeareax2 %>% as.data.frame() %>% add_column("log_rangeareax2")

log_rangeareax1_2 <- predict(lm16, data.frame(log_rangearea = meanlogx1_2, plant = 0.3571, nserve_pop_stability = 4 , private_land_proportion = 0.4871))
log_rangeareax1_2 <- exp(log_rangeareax1_2)
log_rangeareax1_2 <- log_rangeareax1_2 %>% as.data.frame() %>% add_column("log_rangeareax1_2")

private_land_proportion_1 <- predict(lm16, data.frame(log_rangearea = meanlog, plant = 0.3571, nserve_pop_stability = 4 , private_land_proportion = (1)))
private_land_proportion_1 <- exp(private_land_proportion_1)
private_land_proportion_1 <- private_land_proportion_1 %>% as.data.frame() %>% add_column("private_land_proportionx2")

private_land_proportionx1_2 <- predict(lm16, data.frame(log_rangearea = meanlog, plant = 0.3571, nserve_pop_stability = 4 , private_land_proportion = (0.4871 * .5)))
private_land_proportionx1_2 <- exp(private_land_proportionx1_2)
private_land_proportionx1_2 <- private_land_proportionx1_2 %>% as.data.frame() %>% add_column("private_land_proportionx1_2")

##put together dataframe

predictions <- base %>% full_join(plant_0) %>% full_join(plant_1) %>% full_join(log_rangeareax2) %>% full_join(log_rangeareax1_2) %>% full_join(private_land_proportion_1) %>% full_join(private_land_proportionx1_2)
...
names(predictions) <- c(
 "value"          ,                     "\"base\""    ,                    
 "\"plant_0\""          ,                        
 "\"plantx2\""           ,          "\"log_rangeareax2\""      ,      
 "\"log_rangeareax1_2\""    ,       "\"private_land_proportionx1\""  ,
  "\"private_land_proportionx1_2\"" , "call" 
)

predictions <- predictions[,c(1,9)]

##predictions <- predictions %>% select(value, call)
## Can't knit this line ^^ but this gets final dataset 
...
- number of inititives for all species 
```{r}
...
stargazer(lm10, type = "html", title = "Preemptive Conservation Models",  out = "output/format_triala.html", 
           star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)", "Total threats",  "Proportion of private land", "Constant"),
          dep.var.labels=c("Log of No. Initiatives"), 
          add.lines=list(c("AIC", round(AIC(lm10),1)))) 

 
stargazer(lm16, type = "html", title = "Preemptive Conservation Models",  out = "output/format_trialb.html", 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)",  "Natureserve popn. stability", "Proportion of private land", "Constant"),
          dep.var.labels=c("Log of No. Partners"), 
          add.lines=list(c("AIC", round(AIC(lm16),1)))) 

summary(reg_prdctr_init$no_iniv)
sd(reg_prdctr_init$no_iniv)

summary(partners$NoHCPS_count)
sd(partners$NoHCPS_count)
...
stargazer(lm10, lm16,
          ...
          align = TRUE, 
          out = "output/BothLMTable3.html", 
          flip = TRUE, # AIC isn't calculated for lm models so adding manually
          add.lines=list(c("AIC", aic_calc(lm10), aic_calc(lm16)  )) , 
          se = lapply(mdls, se_robust),  ## setting se to be calculated by Robust Standard Errors from function
          omit.stat = c("f", "ser"), 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)", "Total threats", "NatureServe popn stability",  "Proportion of private land","Constant"),
          dep.var.labels=c("Log of No. Initiatives", "Log of No. Partners") )
...
### Changing to capitals
all1[which(all1$one == "preempt"),1] <- "Proactive"
all1[which(all1$one == "react"),1] <- "Reactive"
all1[which(all1$one == "private"),1] <- "Private"
all1[which(all1$one == "public"),1] <- "Public"

all1[which(all1$two == "yes"),2] <- "Yes"
all1[which(all1$two == "no"),2] <- "No"



faded <- ggplot(all1) + aes(x=factor(one), y=all, fill = (two)) +
  geom_bar(stat="identity", position = "stack", alpha = 1/5) +
  scale_y_continuous(name = "Count")+
  labs(fill = "Effort for Species?") + 
  scale_fill_manual(values=c("mediumpurple", "#00abff"))+ 
 scale_x_discrete(name = "Effort Category", limits = c("Proactive", "Reactive", "Private", "Public")) +    theme(text=element_text(color="black", size = 15),axis.text=element_text(color="black"))

ggsave(filename = "fadedESA.png",faded , width = 10, heigh = 5, device = 'png')

sharp <- ggplot(all1) + aes(x=factor(one), y=all, fill = (two)) +
  geom_bar(stat="identity", position = "stack") +
  scale_y_continuous(name = "Count")+
  labs(fill = "Effort for Species?") + 
  scale_fill_manual(values=c("mediumpurple", "#00abff"))+ 
...
ggsave(filename = "sharpESA.png", sharp, width = 10, heigh = 5, device = 'png')
...
### Partner Type Table 
Question here - do we want to use all data that has been sampled or only the 42 species we have in the other analyses? 
Depending on answer, need to change the == "R" to == "X" (same set up as further above) or do some other subsetting 

New note - currently have HCPs in partner type counts (these were not used in regression)
```{r}
part_type <- initiatves_partners2

part_type <- part_type[which(part_type$post_5_25 == "x" | part_type$post_5_25 == "X"),] # need to use the same subset as the rest of the analysis 

part_type <- part_type[-which(is.na(part_type$RowSpecInfo)),]


#also need to remove two species that have data for but which fall outside our dataset
#part_type <- part_type[-which(part_type$post_5_25 == "R"),]

part_type <- part_type %>% dplyr::select(scientific_name, common_name, RowSpecInfo, Agreement_Name, Partner_Source, Partners, Type_of_Org, DocCategory )

part_type <- part_type[-which(part_type$DocCategory == "HCP"),] ## loose ~90 rows 

#Need to remove empty rows with no species information (Before used "X" to filter to rows being used)
#part_type <- part_type[-which(is.na(part_type$common_name)),]

# Remove rows with no partners 
part_type <- part_type[-which(is.na(part_type$Partners)),]

# Remove rows with multi-partner strings 
part_type <- part_type[-which(part_type$Type_of_Org == "multi"),]

# Remove rows with unclear labels
part_type <- part_type[-which(part_type$Type_of_Org == "UNC"),]


## First want to check have the same partner type assigned to each partner
check_here <- part_type %>% group_by(Partners, Type_of_Org) %>% arrange(Partners)

## Also need to check why getting 44 species for USFWS when I think dataset only has 42 species 

check_here_2 <- part_type %>% filter(Type_of_Org == "USFWS") %>% group_by(common_name) %>%    summarise(unique(common_name))


 
## Get unique number of initiatives for each partner type 

no_init_part_type <- part_type %>% 
  group_by(common_name, RowSpecInfo, Agreement_Name, Type_of_Org ) %>% 
  dplyr::count(Type_of_Org) %>%  #for each unique inititive done for a particular species, how many org types are working on that initiative
  group_by(Type_of_Org) %>% 
  dplyr::count(Type_of_Org) # given the unique number of initiatives calculated above, how many initiatives is an org type working on across the dataset
  
## Get unique number of species for each partner type 
no_sp_part_type <- part_type %>% 
  group_by(common_name, Type_of_Org ) %>% 
  dplyr::count(common_name) %>% 
  dplyr::count(Type_of_Org) %>% 
  group_by(Type_of_Org) %>% 
  dplyr::count()
#for each unique inititive done for a particular species, how many org types are working on that initiative
  
count(unique(part_type$scientific_name))



### ways to skin a cat... other options for categories here
...
part_type_cat2 <- part_type

## change all state abbreviations 
part_type_cat2 <- part_type_cat2 %>%
  mutate(Type_of_Org = case_when(str_detect(Type_of_Org, 'SB|SG|SL|SO|SW') ~ 'State', 
                           str_detect(Type_of_Org, 'M|FO|BLM|NPS|USFWS|USFS') ~ 'Federal',
                           str_detect(Type_of_Org, 'P') ~ 'Private',
                           TRUE ~ 'OTHER'))




no_init_part_type_cat2 <- part_type_cat2 %>% 
  group_by(common_name, RowSpecInfo, Agreement_Name, Type_of_Org ) %>% 
  dplyr::count(Type_of_Org) %>%  #for each unique inititive done for a particular species, how many org types are working on that initiative
  group_by(Type_of_Org) %>% 
  dplyr::count(Type_of_Org) # given the unique number of initiatives calculated above, how many initiatives is an org type working on across the dataset
...
## Get unique number of species for each partner type 
no_sp_part_type_cat2 <- part_type_cat2 %>% 
  group_by(common_name, Type_of_Org ) %>% 
  dplyr::count(common_name) %>% 
  dplyr::count(Type_of_Org) %>% 
  group_by(Type_of_Org) %>% 
...
## Get raw count of each partner type 
no_raw_part_type_cat2 <- part_type_cat2 %>% 
  group_by(Type_of_Org ) %>% 
  dplyr::count(Type_of_Org) 

### UGH this has reduancaies in it... Need to find where got number 326 from... 




###### Checking to make sure that the data used here yeilds same results as used in regression 

part_type

dupNoHCPS <- NoHCPS

#dupNoHCPS <- dupNoHCPS[-which(dupNoHCPS$DocCategory == "HCP"),] ## loose ~90 rows 

NoHCPS_count <- dupNoHCPS %>% group_by(scientific_name) %>% 
  distinct(Partners) %>%  dplyr::summarise(n()) 

part_type <- part_type %>% dplyr::select(scientific_name, common_name, RowSpecInfo, Agreement_Name, Partner_Source, Partners, Type_of_Org, DocCategory )


part_typeugh <- part_type[-which(part_type$DocCategory == "HCP"),] ## loose ~90 rows 

part_typeugh <- part_type %>% group_by(scientific_name) %>% 
  distinct(Partners) %>%  dplyr::summarise(n()) 

names(part_typeugh) <- c("scientific_name", "okeycount"  )

checkmeshit <- full_join(part_typeugh, NoHCPS_count)



#######################################################################################################################
### Get same info for no initatives 

no_raw_init_type_cat2 <- part_type_cat2 %>% 
  group_by(Agreement_Name, RowSpecInfo ) %>% 
  dplyr::count(RowSpecInfo) 

no_raw_init_type_cat2 <- no_raw_init_type_cat2 %>% 
  group_by(RowSpecInfo ) %>% 
  dplyr::count(RowSpecInfo) 
...
Difference in data count is that some species don't have partners... 
...
ohboy <- sub_initiatves_partners

ohboy <- ohboy[-which(ohboy$DocCategory == "HCP"),] ## loose ~90 rows 

# Remove rows with no partners 
ohboy <- ohboy[-which(is.na(ohboy$Partners)),]

# Remove rows with multi-partner strings 
ohboy <- ohboy[-which(ohboy$Type_of_Org == "multi"),]

# Remove rows with unclear labels
ohboy <- ohboy[-which(ohboy$Type_of_Org == "UNC"),]

ohboy2 <- ohboy %>% 
  dplyr::select(scientific_name, common_name, RowSpecInfo, Agreement_Name, Partner_Source, Partners, Type_of_Org, DocCategory ) %>% 
  group_by(common_name, RowSpecInfo, Agreement_Name) %>% distinct(Partners) %>%    #count how many names are distinct 
dplyr::summarise(n())

names(ohboy2) <- c("common_name"  ,  "RowSpecInfo"   , "Agreement_Name", "ohboycount"   )

part_type2 <- part_type %>% 
  dplyr::select(scientific_name, common_name, RowSpecInfo, Agreement_Name, Partner_Source, Partners, Type_of_Org, DocCategory ) %>% 
  group_by(common_name, RowSpecInfo, Agreement_Name) %>% distinct(Partners) %>%    #count how many names are distinct 
dplyr::summarise(n())


ugh <- full_join(ohboy2,part_type2)

## So issue is that some of the agreement types were NA but haven't been removed yet 
# Issue ARCTIC GRAYLING- UPPER MISSOURI RIVER DPS - - Big Hole River Drought Management Plan
  # Need to use initiatves_partners2 (I think original dfs used to create ohboy2 vs part_type2 differ and that's causeing issue)

getnumbers <- ugh %>% group_by(common_name, RowSpecInfo) %>% dplyr::summarise(n())
...
```{r scrap, include = FALSE, echo = FALSE, eval=FALSE, warning=FALSE}
...
#checkdf <- checkdf[-which(is.na(checkdf$Initiatives_confidence_measure)),] ## This is where the issue is coming from (only have 38 species here... Let's try and do the same thing but a different way)


checkdf <- checkdf %>% dplyr::select(scientific_name, common_name, Initiatives_confidence_measure, partners_confidence_measure)

checkdf <- full_join(checkdf, names, by = c("scientific_name", "common_name"  ))

### ugh okay so still missing data for some species but I have it in the data sheet so just going to manually add it to the spreadsheet..

# First remove duplicated species
checkdf <- checkdf[-c(6),] 

checkdf[24,c(3:4)] <- c("RC")
checkdf[31,c(3:4)] <- c("MI")
checkdf[33,c(3:4)] <- c("SI")
checkdf[38,c(3:4)] <- c("RC")



checkdf <- checkdf %>%  mutate(partner_good_5 = case_when(grepl("RC", partners_confidence_measure, ignore.case = TRUE) ~ 1))  %>% 
  replace(is.na(.), .5)

checkdf <- checkdf %>%  mutate(Initiatives_good_5 = case_when(grepl("RC", Initiatives_confidence_measure, ignore.case = TRUE) ~ 1))  %>% 
  replace(is.na(.), .5)

checkdf <- checkdf %>%  mutate(partner_good_1_5 = case_when(grepl("RC", partners_confidence_measure, ignore.case = TRUE) ~ 1))  %>% 
  replace(is.na(.), 1.5)

checkdf <- checkdf %>%  mutate(Initiatives_good_1_5 = case_when(grepl("RC", Initiatives_confidence_measure, ignore.case = TRUE) ~ 1))  %>% 
  replace(is.na(.), 1.5)
...
### normal
lma1 <- lm(log(NoHCPS_count) ~ log(range_area) + plant  + nserve_pop_stability + private_land_proportion,data = useme)

### .5
lmb1 <- lm(log(NoHCPS_count) ~ log(range_area) + plant  + nserve_pop_stability + private_land_proportion,data = useme, weights=partner_good_5)
summary(lma1)

### 1.5
lmc1 <- lm(log(NoHCPS_count) ~ log(range_area) + plant  + nserve_pop_stability + private_land_proportion,data = useme, weights=partner_good_1_5)



useme2 <- full_join(reg_prdctr_init, checkdf)



### normal
lma2 <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion,data = useme2)

### .5
lmb2 <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion,data = useme2, weights=Initiatives_good_5)

### 1.5
lmc2 <- lm(log(no_iniv) ~ log(range_area) + plant  + total_threats + private_land_proportion,data = useme2, weights=Initiatives_good_1_5)

stargazer(lma1, lmb1, lma2, lmb2, type = "html",  out = "output/WeightedConfidenceCheck_2022_04_22.html", 
          star.char = c("+", "*", "**", "***"),
          star.cutoffs = c(.1, .05, .01, .001), 
          notes.append = FALSE, 
          notes = c("+ p<.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          covariate.labels=c("Log of range area", "Plant (present = 1)",  "Natureserve popn. stability", "Total threats", "Proportion of private land", "Constant"),
          column.labels = c("PartnerNormal", "Partner.5", "InitiativesNormal", "Initiatives.5"),
          model.numbers = FALSE,
          dep.var.labels=c("Log of No. Partners", "Log of No. Initiatives"),
          add.lines=list(c("AIC", round(AIC(lma1),1), round(AIC(lmb1),1),  round(AIC(lma2),1), round(AIC(lmb2),1) )))
...
Becky additional figure to add
```{r}
...
plot(reg_partners$range_area, reg_partners$NoHCPS_count)
abline((lm16))

## manually drop obs that aren't in model
reg_partners_mod_mod <- reg_partners

reg_partners_mod_mod <- reg_partners_mod_mod[-which(is.na(reg_partners_mod_mod$log_rangearea)),]
reg_partners_mod_mod <- reg_partners_mod_mod[-which(is.na(reg_partners_mod_mod$NoHCPS_count)),]
reg_partners_mod_mod <- reg_partners_mod_mod[-which(is.na(reg_partners_mod_mod$nserve_pop_stability)),]


plot( predict(lm16), reg_partners_mod_mod$NoHCPS_count)
plot_data <- data.frame( predicted_data = predict(lm16), reg_partners_mod_mod$NoHCPS_count)



plot_data <- data.frame(Predicted_value = predict(lm16),  
                       Observed_value = reg_partners_mod_mod$log_HCPS, prop = reg_partners_mod_mod$private_land_proportion)

#plot_data$Predicted_value <- exp(plot_data$Predicted_value)

plot(plot_data$prop, plot_data$Observed_value)
abline(plot_data$prop, plot_data$Predicted_value)





plot_data2 <- data.frame(Predicted_value = predict(lm16),  
                       Observed_value = reg_partners_mod_mod$log_HCPS, lgrange = reg_partners_mod_mod$log_rangearea)

#plot_data2$Predicted_value <- exp(plot_data2$Predicted_value)

plot(plot_data2$lgrange, plot_data2$Observed_value)
abline(reg = plot_data2$lgrange ~ plot_data2$Predicted_value)
...
## manually drop obs that aren't in model
reg_init_mod_mod <- reg_prdctr_init

reg_init_mod_mod <- reg_init_mod_mod[-which(is.na(reg_init_mod_mod$log_rangearea)),]




plot_data <- data.frame(Predicted_value = predict(lm10),  
                       Observed_value = reg_init_mod_mod$log_initatives, prop = reg_init_mod_mod$private_land_proportion)
...
plot_data <- data.frame(Predicted_value = predict(lm10),  
                       Observed_value = reg_init_mod_mod$log_initatives, logrange = reg_init_mod_mod$log_rangearea)

#plot_data$Predicted_value <- exp(plot_data$Predicted_value)


x <- (1:length(reg_init_mod_mod$log_rangearea))

plot(plot_data$logrange, plot_data$Observed_value)
abline(plot_data$logrange, plot_data$Predicted_value)

#add linear trend
lines(predict(lm(plot_data$Predicted_value~plot_data$logrange)),col='green')
...
```{r OLD heteroskedasticity, include = FALSE, echo = FALSE, eval=FALSE}
bptest(lm10)
