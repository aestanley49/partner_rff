...
reg_prdctr <- reg_prdctr %>% left_join(FR_pg_length)

## Add number of partners 

reg_prdctr <- reg_prdctr %>% left_join(mod_partner_count)
...
#set up
PredictorsOnlyPixel <- reg_prdctr[,c(3)]
...
vif(lm(no_iniv ~ range_area +percentpublic + plant + public_mang_shannons_diversity + total_threats + FR_pg_length + patner_count,data = reg_prdctr))
```

Q - keep log??? 

```{r lm1}
lm2 <- lm(log(no_iniv) ~ range_area +percentpublic + plant + public_mang_shannons_diversity + total_threats,data = reg_prdctr)

#withou log 
#lm2 <- lm((no_iniv) ~ range_area +percentpublic + plant + public_mang_shannons_diversity + total_threats,data = reg_prdctr)

### ### ### ### ### ### ### ###

lm2 <-  lm(log(no_iniv) ~ range_area +percentpublic + plant + public_mang_shannons_diversity + total_threats + FR_pg_length + patner_count,data = reg_prdctr)
...
### ### So need to add all remaining partners, re run and then decide on bins 
