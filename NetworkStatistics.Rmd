...
###Version 1: Create smallest version of chi square 
```{r}
simpledata <- g
...
privateother <- partnertypeactions[c(2,5,7),-c(1)]
privateother <- privateother %>%  bind_rows(summarise(., across(where(is.numeric), sum), across(where(is.character), ~ "Total")))
federal_other <- partnertypeactions[c(3,4),]
...
corrplot(contrib, is.cor = FALSE)
```
Note - violate chi square assumption here.. re make data table with new combinations?? 


Make new combinations of org types here as well... 

V1 - smallest combination possible 
```{r}
partnertypeactions <- finaldf
partnertypeactions <- partnertypeactions[,c(4:14,16)] #only selecting partner types and actions
