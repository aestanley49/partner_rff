...
Create new orgtype_adjmat where combine N, C and R and add M to FO
...
tlist <- simp_tadjmat %>% melt() 
tnet <- graph_from_data_frame(d=subset(tlist, value>0), directed=F) 
...
```
Q - why are the calculations treating all nodes the same? 
...
set up so partner types on rows and actions in cols
```{r}
...
colnames(partnertypeactions) <- c("Land Water Management","Species Management", "Awareness", "Law","Econ","Conservation Planning", "Policy", "Research", "Education","Instiutional Development", "Funding")

chisq <- chisq.test(partnertypeactions)
chisq
