...
#Total number of partners
length(names(PartnersData[,-c(1:2)])) # I counted how many columns there were if i didn't look at columns 1 and 2 ## 200 unique partners
...
```{r, echo= FALSE, size='tiny'}
# the number of partners working on each species:
rowSums(PartnersData[,-c(1:2)]) # this gives a not very intuitive vector so I put it in the dataset

PartnersData$TotalPartners <- rowSums(PartnersData[,-c(1:2)])
PartnersData[,c(2,203)] # this is the dataset with names and numbers of species. It does not print out in a convenient format. probably should save and make a better table.

# Histogram of partners per species
hist(PartnersData$TotalPartners,
     xlab ="number of partners working on a species",
     breaks = 60,
     main = NULL)

#	Max, min, median, and 1st and 3rd quantiles across species
summary(PartnersData$TotalPartners)
...
write.csv(colSums(PartnersData[,-c(1:2)]),paste(output.dir,"/PartnersList.csv", sep = "")) # not very well formatted so I just write to file 

#	Histogram of species/partner 
hist(colSums(PartnersData[,-c(1:2)]),
     xlab ="number of species that a partner works on",
     breaks = 50, 
     main = NULL)

# table of this info seems more useful
table(colSums(PartnersData[,-c(1:2)]))

# summary of species/partner
summary(colSums(PartnersData[,-c(1:2)])) # 372 Max seems wrong - this is a sum of the colsums, unclear why this is being added | if data is reloaded at begining of section, don't have this problem
...
```{r}
Partner <- as.matrix(PartnersData[,-c(1:2)])
#Data <- t(Partner)
#View(Data)
