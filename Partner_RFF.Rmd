...
#Total number of partners
length(names(PartnerData[,-c(1:2)])) # I counted how many columns there were if i didn't look at columns 1 and 2 ## 242 unique partners
#length(unique(names(PartnerData[,-c(1:2)]))) # just making sure that there are no repeats. ## 242 so its ok
```
...
PartnerData[,c(2,244)] # this is the dataset with names and numbers of species. It does not print out in a convenient format. probably should save and make a better table.
...
summary(colSums(PartnerData[,-c(1:2)])) # 372 Max seems wrong - this is a sum of the colsums, unclear why this is being added | if data is reloaded at begining of section, don't have this problem
...
write.csv(adjmat, "/usr/local/bin/store/partner_rff/data/adjacencymatrix.csv")
