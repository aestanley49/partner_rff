...
PartnerData <- FormatData(RawData) # this wasn't used in later data processes 
partnercount <- read.csv(paste0(DataSource,"/EditedFormatDataOutputwithTotals.csv"), stringsAsFactors = FALSE) #this file is writen at the end of the FormatData(RawData) function. Maybe had error getting function to load in so wanted to take final csv instead? 
...
center <- read_excel(paste0(DataSource, "/mean_center_range_subsetofprecluded_with_partnerdata.xlsx")) #made excel by calculating centroids in ccegeomaps that cornell had and combining all values into a spreadsheet so this is the raw data from that 
...
#write to csv to read into another script 
write.csv(combo,paste0(DataSource,"/tylerdataandpartnercounts", sep = ""))
...
```{r income, echo= FALSE, include= FALSE, eval=FALSE}
#view distribution of employment
...
ugh <- combo
ugh <- ugh[,c(1,5)]
okay <- ugh %>% full_join(center, by = "scientific_name")
...
okay <- okay[-which(is.na(okay$area_x)),] #for some reason this value changed without me changing the value? 
...
#remove percent federal land
combo <- combo[,c(1:7,9:11)]
#having isses getting Chorizanthe parryi var. fernandina total to stick so just going to write that in here
combo[1,5] <- 2
...
#change order 
combooo <- combooo[,c(1:4,6:10,5)]

#set up
PredictorsOnlyPixel <- combooo[,c(10)]
...
testcor <- combo[,-c(1:4)]
var(testcor, na.rm = TRUE) ## this computes all correlations don't actually need manual compuations below 
...
```{r}
write.csv(new,paste(DataSource,"/newdf2.csv", sep = ""))
```


variance inflation factors : Detecting multicollinearity
```{r VIFs}
new <- combo[,-c(1:4)] # have csv file in "OutputandInputforThesis" folder and i think this was when newdf.csv was written 
...
```{r glm scrap work, eval=FALSE, echo=FALSE, include=FALSE}

#need to fix NA in partnercounts (why is that there now?)
#then run binomial and possion 
#read through chapter 11 

#remove NA from parameter 
new1 <- new[-which(is.na(new$total)),]


fit_tnbinom <- glmmTMB(Y~ X1  + X2 + X3 + X4, + X5, data=new,ziformula=~0,  family=list(family="truncated_nbinom1",link="log"))

##need to include total_x in data
fit_tnbinom <- glmmTMB(total ~ taxa + percentpublic + area_x + nsumemploy + total_x_x, data=new, ziformula=~0, family=list(family="truncated_nbinom1",link="log"))

fit_tnbinom <- glmmTMB(total~ taxa, data=new, ziformula=~0, family=list(family="truncated_nbinom1",link="log"))
...
fit_zipoisson <- glmmTMB(total~ area_x + percentpublic + taxa + nsumemploy + nsumemploy + total_x_x, data=new, ziformula=~0, family=list(family="",link="log"))

fit_zipoisson <- glmmTMB(total~ taxa + nsumemploy, data=new1, family=truncated_poisson(link = "log"))

fit_zipoisson <- glmmTMB(total~ taxa + nsumemploy, data=new1, family=gaussian())

fit_zipoisson <- glmmTMB(total~ taxa + nsumemploy, data=new1, family=poisson)
...
#start again from conversation with gwen and becky 

# unsure how to address NaN function evaluation so lets try removing all rows with NAs
new1 <- new1[-which(is.na(new1$percentpublic)),]
  # still get same warning when all NA values are removed so must be something else 

gaussian <- glmmTMB(total ~ taxa + percentpublic + area_x + nsumemploy + total_x_x, data=new1, family=gaussian())
  #runs with warning - "Model convergence problem; extreme or very small eigen values detected"

summary(gaussian)

#numbers too big? remove area and nsumemploy
gaussian <- glmmTMB(total ~ taxa + percentpublic + total_x_x, data=new1, family=gaussian())


fit_tpoisson <- glmmTMB(total ~ taxa + percentpublic + area_x + nsumemploy + total_x_x, data=new1, family=genpois(link = "log"))

#numbers too big? remove area and nsumemploy
fit_tpoisson <- glmmTMB(total ~ taxa + percentpublic + total_x_x, data=new1, family=genpois(link = "log"))

fit_tpoisson <- glmmTMB(total ~ taxa + percentpublic + area_x + nsumemploy + total_x_x, data=new1, family=truncated_poisson(link = "log"))
  #try specifiying that issue with NA

#numbers too big? remove area and nsumemploy
fit_tpoisson <- glmmTMB(total ~ taxa + percentpublic + total_x_x, data=new1, family=truncated_poisson(link = "log"))
  #try specifiying that issue with NA


fit_tnbinom <- glmmTMB(total ~ taxa + percentpublic + area_x + nsumemploy + total_x_x, data=new1, family=nbinom1(link = "log"))
  #got the lgamma value out of range error 

fit_tnbinom <- glmmTMB(total ~ taxa + percentpublic + area_x + nsumemploy + total_x_x, data=new1, family=truncated_nbinom1(link = "log"))
  #got the lgamma value out of range error 

fit_tnbinom <- glmmTMB(total ~ taxa + percentpublic + total_x_x + nsumemploy, data=new1, family=truncated_nbinom1(link = "log"))
```


```{r glmm, eval=FALSE}
# so issue is that numbers in area and nsum are too big 
# current units : 
    # Area.x = total area in m^2 of the counties the species occupies
    # nsumeploy = The average, area weighted per county, level of employment across all sectors (in number of jobs) for all counties a species exists in. 
# so if apply same metric to area, should be okay to apply to nsumeploy? 
# *** making big assumption here, try and check with tyler

#nevermind, the dispartiy between the two numbers are two great and I'm not 100% sure how to recalculate nsumeploy 
#going to drop this parameter for now and recalculate area 
#lets try changing it to hectares or 10,000 m2

new <- combo[,-c(1:4)]
#remove NA from parameter 
new1 <- new[-which(is.na(new$percentpublic)),]
new2 <- new1 %>% mutate(narea = area_x / 1000000000)
#new2 <- new2[,-c(2)] #remove irrelevant columns 


#try again with regression 
  # all worked with "taxa + percentpublic + total_x_x" as predictors 
#adding narea 
  #for gaussian at original m2/10,000 get model vonvergence problem 
  # trying new narea - m2/10,000,000 --> convergence issue with all but gaussian 
  # trying new narea - m2/10,000,000,000 - no issue with convergence 
  #trying new narea - m2/10,000,000,00 - no issue with convergence ***
  #trying new narea - m2/10,000,000,0 - issue with general pois

gaussian <- glmmTMB(total ~ taxa + percentpublic + total_x_x + narea, data=new2, family=gaussian())

fit_genpoisson <- glmmTMB(total ~ taxa + percentpublic + total_x_x + narea, data=new2, family=genpois(link = "log"))
  # some issue with convergence 

fit_tpoisson <- glmmTMB(total ~ taxa + percentpublic + total_x_x +narea, data=new2, family=truncated_poisson(link = "log"))

qqPlot(residuals(fit_tpoisson))

fit_binom <- glmmTMB(total ~ taxa + taxa + percentpublic + total_x_x + narea, data=new2, family=nbinom1(link = "log"))
  #got the lgamma value out of range error 

fit_tnbinom <- glmmTMB(total ~ taxa + percentpublic + total_x_x + narea, data=new2, family=truncated_nbinom1(link = "log"))
