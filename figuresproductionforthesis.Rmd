...
```{r}
#combine newdfeast with newdfwest and "and"

all <- and
all <- all %>% rename("id" = X1)
newdfwest <- newdfwest %>% rename("westsum" = rssum)

newdfeast <- newdfeast%>% rename("eastsum" = rssum)

all <- all %>% left_join(newdfwest, by = "id") %>% left_join(newdfeast, by = "id")

data <- all
View(all)

data %>% group_by(westsum, eastsum) %>% 
ggplot(data = all, mapping = aes(x=partnerships_per_partner, y= species_per_partner)) + geom_count() +coord_flip()
