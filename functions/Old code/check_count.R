...
library(tidyverse)
library(janitor)
counts <- read_excel("/usr/local/bin/store/partner_rff/data/archive/loose excel sheets/11_3_19partnershipdata_cleaned_AS7.xlsx", "notes on each doc")
...
write.csv (mydata, "~/Desktop/AS7excel.csv")
...
both <- both[, c(1,2,8,9,13,15,17)]

both$


###########################################################################having issues running code here 
...
same <- both %>% filter(both$total[] == both$total_number_of_partners_for_species[])
#View(same)
...
### ### ### go through and checking differences in between the two rows with partner names
look <- both[,c(2,3,6)]
look <- look[-c(1,2,5,6,9,11,13,16,21,28:30,39,41,47,54,58),] ##removed columns that had exactly the same names 
look <- look[-which(is.na(look$who_the_partners_are)),]
#remove rows with NAs
View(look)
  ## note CORAL PINK SAND DUNES TIGER BEETLE has USFWS as FWS


### to do -
  ### need to find out how to check full text inside cell
  # cross check against previously stated discrpeancies 
  # make final list 
